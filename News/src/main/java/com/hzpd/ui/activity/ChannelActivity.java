...
import com.hzpd.adapter.OtherAdapter;
 ...
 * 频道管理
 */
public class ChannelActivity extends MBaseActivity implements OnItemClickListener {
	...
	/** 用户栏目对应的适配器，可以拖动 */
	DragAdapter2 userAdapter;
	/** 其它栏目对应的适配器 */
	OtherAdapter otherAdapter;
	/** 其它栏目列表 */
	ArrayList<NewsChannelBean> otherChannelList = new ArrayList<NewsChannelBean>();
	/** 用户栏目列表 */
	ArrayList<NewsChannelBean> userChannelList = new ArrayList<NewsChannelBean>();
...
//		userChannelList = ((ArrayList<NewsChannelBean>)ChannelManage.getManage(AppApplication.getApp().getSQLHelper()).getUserChannel());
//		otherChannelList = ((ArrayList<NewsChannelBean>)ChannelManage.getManage(AppApplication.getApp().getSQLHelper()).getOtherChannel());
		...
		userAdapter = new DragAdapter2(this, channelData);
		...
		switch (parent.getId()) {
			case R.id.userGridView:
				...
				if (position != 0 && position != 1) {
					...
					if (moveImageView != null) {
						...
						final NewsChannelBean channel = ((DragAdapter2) parent.getAdapter()).getItem(position);//获取点击的频道内容
			...
			case R.id.otherGridView:
				...
				if (moveImageView != null){
					...
					final int[] startLocation = new int[2];
					newTextView.getLocationInWindow(startLocation);
					final NewsChannelBean channel = ((OtherAdapter) parent.getAdapter()).getItem(position);
	 ...
	 * @param clickGridView
	 */
	private void MoveAnim(View moveView, int[] startLocation,int[] endLocation, final NewsChannelBean moveChannel,
