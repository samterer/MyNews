...
public class ZhuantiListDbTask {
	private DbUtils zhuantiListDb;

	public ZhuantiListDbTask(Context context) {
		zhuantiListDb = DBHelper.getInstance(context).getZhuantiListDbUtils();
	}
	...
	class NewsFindTask extends AsyncTask<String, String, List<NewsBeanDB>> {
		...
		public NewsFindTask(int page, int pageSize, String sid
				, I_SetList<NewsBeanDB> callBack) {
			this.page = page - 1;
